...
export UE_VAR=ue
if [ -n "$2" ] && [ -n "$3" ]; then
    sudo -E .././srsran/build/srsue/src/srsue "$1" "$2" --rf.device_name=zmq --rf.device_args="tx_port=tcp://*:2001,rx_port=tcp://localhost:2000,id=ue,base_srate=23.04e6" --gw.netns=ue1 > ./tmp/ue_logger 2>&1
else
